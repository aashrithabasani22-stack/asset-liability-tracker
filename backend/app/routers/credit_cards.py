...
def _get(db, item_id, user_id):
    item = db.query(models.CreditCard).filter(models.CreditCard.id == item_id, models.CreditCard.user_id == user_id).first()
    if not item: raise HTTPException(404, "Not found")
    return item

@router.post("", response_model=schemas.CreditCardOut)
def create(payload: schemas.CreditCardCreate, db: Session = Depends(get_db), user=Depends(get_current_user)):
    item = models.CreditCard(**payload.model_dump(), user_id=user.id)
...
@router.put("/{item_id}", response_model=schemas.CreditCardOut)
def update(item_id: int, payload: schemas.CreditCardCreate, db: Session = Depends(get_db), user=Depends(get_current_user)):
